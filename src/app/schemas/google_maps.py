...
class AddressPredictionRequest(BaseModel):
    ...
    """
    input: str = Field(..., description="Text to get address predictions for", min_length=2)
...
class StructuredFormatting(BaseModel):
    ...
    secondary_text: Optional[str] = Field(None, description="Secondary text of the prediction")
...
class AddressPredictionResponse(BaseModel):
    ...
    """
    predictions: List[AddressPrediction] = Field(..., description="List of address predictions")
